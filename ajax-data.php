...
if (!empty($_POST) ) {
    ...
    function getData($sql){		
        ...
        foreach ($query as $row ) {						
            $row['edit_link'] = '<a href="'.$row['enquiry_id'].'">View</a>';
			$data[] = $row ;
        }		
        return $data;
    }
    ...
    $recordsTotal = 10;//count(getData("SELECT * FROM ".MyTable));
    ...
    if(!empty($_POST['search']['value'])){
        ...
        $recordsFiltered = 10;//count(getData($sql));//Count of search result
