...
class Userprofilelist extends CI_Controller {
...
//        die;
        ...
        if (isset($_POST)) {
            ...
            $this->userprofile_model->getAjaxData();
        }
    }
    ...
    public function deleteUserList() {
        ...
        if (isset($_POST)) {
            $this->load->model('userprofile_model');
            $admin_ids = $this->input->post('ids');

            if (isset($admin_ids)) {
                if ($this->userprofile_model->getAjaxDeleteFromUserList($admin_ids)) {
