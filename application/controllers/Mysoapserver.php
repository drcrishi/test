...
//        );
        ...
        $this->nusoap_server->register(
                "setContactEnquiryId", array("contactData" => "tns:setEnqiryDataArrayStruct"), array("id" => "xsd:integer"), "urn:MySoapServer", "setContactData", "rpc", "encoded", ""
        );
...
//            fclose($fp);
            list($movingFromSuburb, $movingFromPostCode, $movingFromState, $movingFromStreet) = explode(",", $moverDataArray['movingfrom']);
            list($movingToSuburb, $movingToPostCode, $movingToState, $movingToStreet) = explode(",", $moverDataArray['movingto']);
            ...
            /* INSTANT QUOTE....@DRCZ */
            $paymentInfo = explode('###', $moverDataArray['client_info']);
            /* INSTANT QUOTE....@DRCZ */

            $data = array(
                'ms_uuid' => $moverDataArray['m_uuid'],
//                'en_unique_id' => $moverDataArray['m_uuid'],
                ...
                'en_movingfrom_street' => trim($movingFromStreet),
                ...
                'en_movingto_street' => trim($movingToStreet),
                ...
                'customer_id' => $moverDataArray['client_id'],
                'booking_status' => $moverDataArray['booking_status']
            );
            $CI = & get_instance();
            ...
            //for web.............
//          $uuid = $CI->enquiry_model->addEnquirydataWeb($data);
            $notify = array(
                'notification_type' => 1,
                'uuid' => $uuid
            );
            $CI->notify_model->setNotification($notify);
            $enq_id = $CI->enquiry_model->getEnquiryIDFromUUID($uuid);
            $internalnotesData = array(
                'notes_description' => $moverDataArray['notes_description'],
                'notes_attachedfile' => ""
            );
            if ($moverDataArray['notes_description'] != "") {
                $CI->enquiry_model->addOPSNotes($internalnotesData, $enq_id);
            }
            /* INSTANT QUOTE....@DRCZ */
            if ($paymentInfo[2] == "PackerInstantQuotePaymentNotEligible") {
                $handle = fopen("PIQ_LOG", "a");
                fwrite($handle, "EnquiryIdPNE " . $enq_id);

                /* SEND AUTO EMAIL............@DRCZ */
                //  file_get_contents("http://crm.hireamover.com.au/email/sendAutoInstantQuote/".$enq_id."/Quote?auth=slientEmailAhj");
                /* SEND AUTO EMAIL............@DRCZ */
            }
            /* INSTANT QUOTE....@DRCZ */
        ...
        function getMoverEnquiry($enquiryUID) {
            ...
            $returnArray = array(
                "m_uuid" => $enquiryData[0]['ms_uuid'],
                "new_uuid" => $enquiryData[0]['en_unique_id'],
                ...
                "client_id" => $enquiryData[0]['customer_id'],
                "booking_status" => $enquiryData[0]['booking_status'],
                "additional_pickup" => $enquiryData[0]['additional_pickup'],
                "additional_delivery" => $enquiryData[0]['additional_delivery'],
            );
            return $returnArray;
        }
...
//            return $updateMoverDataArray['uuid'];
            if ($updateMoverDataArray['uuid'] != "") {
                ...
                $data = array(
                    'ms_uuid' => $updateMoverDataArray['m_uuid'],
                    'qualified_date' => $updateMoverDataArray['qualified_date'],
                    'booking_status' => $updateMoverDataArray['booking_status'],
                    'customer_id' => $updateMoverDataArray['customer_id'],
                    'en_fname' => trim($firstName),
                    'en_lname' => trim($lastName),
                    ...
                    'en_eway_token' => $updateMoverDataArray['en_eway_token'],
                    'is_qualified' => $updateMoverDataArray['booking'],
                    'additional_pickup' => $updateMoverDataArray['additional_pickup'],
                    'additional_delivery' => $updateMoverDataArray['additional_delivery'],
                    'depositreceiveddate' => $updateMoverDataArray['depositreceiveddate'],
                    'notify_deposit' => '0'
                );
                $result = array_filter($data);

                return $CI->enquiry_model->editEnquiryById($id, $result);
            }
        }
        ...
        function udpateMoverEnquiry1($updateMoverDataArray) {
            if ($updateMoverDataArray['uuid'] != "") {
                ...
                $CI->load->model("enquiry_model");
                $id = $CI->enquiry_model->getEnquiryIDFromUUID($updateMoverDataArray['uuid']);
                $enqData = $CI->enquiry_model->getEnquiryDataByUUID($updateMoverDataArray['uuid']);
                $f_eway = $enqData[0]['final_payment_eway_refno'];
                if(!isset($f_eway)){
                  $ewayfinal = $updateMoverDataArray['final_payment_eway_refno'];  
                }
                else if ($f_eway != '' || $f_eway != NULL || $f_eway != 0) {
                    if($f_eway == 0){$feway = "";}else{$feway = ',' . $f_eway;}
                   $ewayfinal = $updateMoverDataArray['final_payment_eway_refno']. $feway ;
                ...
                $data = array(
                    'final_payment_eway_refno' => $ewayfinal,
...
//            );
        ...
        function setContactEnquiryId($contactDataArray1) {
            $CI = & get_instance();
            log_message("error", "dddd");
            $CI->load->model("contact_model");
