...
//        }
    ...
    public function index() {
        ...
        $data['userprofile'] = $this->userbooking_model->getadminRPPwd($sessionEmail);
    ...
    public function changeUserProfile() {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($this->upload->do_upload('userprofile')) {
                ...
                if ($this->userbooking_model->changeProfilePic($sessionEmail, $userprofile) != FALSE) {
                    echo json_encode(array("success" => 1));
                } else {
                    echo json_encode(array("error" => "<p>Profile not changed.</p>"));
                }
            } else {
                echo json_encode(array("error" => "<p>Supports only jpg,png,jpeg images.</p>"));
            }
        }
    }
